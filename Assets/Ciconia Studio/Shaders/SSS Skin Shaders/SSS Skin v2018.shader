Shader "Ciconia Studio/CS_Skin/Builtin/Skin 2018" {
    Properties {
...
////// Specular:
                ...
                float3 indirectSpecular = (gi.indirect.specular + float3(_SpecularAmbient,_SpecularAmbient,_SpecularAmbient))*specularColor;
...
/////// Diffuse:
                ...
                float node_7778 = 0.0;
                float node_2611 = (1.0+_Contrast);
                float3 node_2165 = (saturate((_TranslucencySpread+(node_2611 + ( (lerp( (1.0 - _Translucencymap_var.rgb), _Translucencymap_var.rgb, _Invertmap ) - node_7778) * ((-1*_Contrast) - node_2611) ) / (1.0 - node_7778))))*_TranslucencyIntensity);
                float3 Transmissionmap = saturate((1.0-(1.0-(max(0,dot(i.normalDir,lightDirection))*attenuation))*(1.0-(_TranslucencyColor.rgb*node_2165))));
                ...
                float3 Visualize = node_2165;
                float3 diffuseColor = lerp( Diffusemap, Visualize, _Visualize );
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float3 node_2165 = (saturate((_TranslucencySpread+(node_2611 + ( (lerp( (1.0 - _Translucencymap_var.rgb), _Translucencymap_var.rgb, _Invertmap ) - node_7778) * ((-1*_Contrast) - node_2611) ) / (1.0 - node_7778))))*_TranslucencyIntensity);
                float3 Visualize = node_2165;
                float3 diffColor = lerp( Diffusemap, Visualize, _Visualize );
                float4 _SpecGlossMap_var = tex2D(_SpecGlossMap,TRANSFORM_TEX(i.uv0, _SpecGlossMap));
                float3 Specularmap = (((0.95*pow(1.0-max(0,dot(normalDirection, viewDirection)),1.0))+0.05)*(_SpecularColor.rgb*_SpecGlossMap_var.rgb*_SpecularIntensity));
