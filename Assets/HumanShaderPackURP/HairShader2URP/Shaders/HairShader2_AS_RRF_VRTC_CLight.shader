...
Shader "RRF_HumanShaders/HairShader2/HairShader2_AS_RRF_VRTC_CLight"
{
	Properties
	{
		_Cutoff( "Mask Clip Value", Float ) = 0.63
		...
		_Alpha_Power("Alpha_Power", Range( 0 , 2)) = 1.8
		_Alpha_Level("Alpha_Level", Range( 0 , 2)) = 0.3
		_VRTC("VRTC", 2D) = "white" {}
		...
		_CustomLightingShadow("CustomLightingShadow", Range( 0 , 1)) = 0.8
		...
		_FresnelColourEffectA("FresnelColour-Effect(A)", Color) = (0.6226415,0.2261481,0.2261481,0)
		_FresnelScale("FresnelScale", Range( 0 , 2)) = 0
		_FresnelBias("FresnelBias", Range( -2 , 2)) = 0
		_FresnelPower("FresnelPower", Range( 0 , 2)) = 0
		_Fresnel_PostEffect("Fresnel_PostEffect", Range( 0 , 0.2)) = 0.2
	...
	SubShader
	{
		...
		uniform float _CustomLightingShadow;
		uniform float _CustomLightingPower;
		uniform float _FresnelBias;
		uniform float _FresnelScale;
		uniform float _FresnelPower;
		uniform float4 _FresnelColourEffectA;
		uniform float _Metallic;
		uniform float _Fresnel_PostEffect;
		uniform float _Cutoff = 0.63;
		...
		inline half4 LightingStandardCustomLighting( inout SurfaceOutputCustomLightingCustom s, half3 viewDir, UnityGI gi )
		{
			...
			float clampResult489 = clamp( pow( ( originalAlphaCutout403 * _Alpha_Level ) , _Alpha_Power ) , 0.0 , 1.0 );
			float Original_HairAlpha392 = clampResult489;
			...
			float lerpResult486 = lerp( 1.0 , ase_lightAtten , _CustomLightingShadow);
			#if defined(LIGHTMAP_ON) && ( UNITY_VERSION < 560 || ( defined(LIGHTMAP_SHADOW_MIXING) && !defined(SHADOWS_SHADOWMASK) && defined(SHADOWS_SCREEN) ) )//aselc
			float4 ase_lightColor = 0;
			#else //aselc
			float4 ase_lightColor = _LightColor0;
			#endif //aselc
			float clampResult456 = clamp( ( lerpResult486 * ase_lightColor.a * ( _CustomLightingPower * 10.0 ) ) , 0.0 , 1.0 );
			float LightAttInt457 = clampResult456;
			SurfaceOutputStandard s448 = (SurfaceOutputStandard ) 0;
			...
			float fresnelNdotV469 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode469 = ( _FresnelBias + _FresnelScale * pow( 1.0 - fresnelNdotV469, _FresnelPower ) );
			float clampResult479 = clamp( fresnelNode469 , 0.0 , 1.0 );
			float4 temp_output_470_0 = ( OUTPUT_Albedo407 + ( ( clampResult479 * ( _FresnelColourEffectA.a * 3.0 ) ) * _FresnelColourEffectA ) );
			s448.Albedo = temp_output_470_0.rgb;
			float3 OUTPUT_NormalMap410 = tex2DNode7;
			s448.Normal = WorldNormalVector( i , OUTPUT_NormalMap410 );
			...
			s448.Emission = OUTPUT_Emissive398.rgb;
			s448.Metallic = _Metallic;
			...
			s448.Smoothness = OUTPUT_Smothness412;
			s448.Occlusion = 1.0;
			...
			UnityGI gi448 = gi;
			#ifdef UNITY_PASS_FORWARDBASE
			Unity_GlossyEnvironmentData g448 = UnityGlossyEnvironmentSetup( s448.Smoothness, data.worldViewDir, s448.Normal, float3(0,0,0));
			gi448 = UnityGlobalIllumination( data, s448.Occlusion, s448.Normal, g448 );
			#endif

			float3 surfResult448 = LightingStandard ( s448, viewDir, gi448 ).rgb;
			surfResult448 += s448.Emission;

			#ifdef UNITY_PASS_FORWARDADD//448
			surfResult448 -= s448.Emission;
			#endif//448
			float3 temp_output_450_0 = ( LightAttInt457 * surfResult448 );
			float4 lerpResult483 = lerp( float4( temp_output_450_0 , 0.0 ) , ( float4( temp_output_450_0 , 0.0 ) + temp_output_470_0 ) , _Fresnel_PostEffect);
			c.rgb = lerpResult483.rgb;
			c.a = temp_output_389_0;
		...
		void surf( Input i , inout SurfaceOutputCustomLightingCustom o )
		{
			...
			float4 temp_output_265_0 = ( ( pow( sinTHL2256 , _SecondaryHighlightExponent ) * _SecondaryHighlightStrength * dirAtten102 * lightZone285 ) * _SecondaryHighlightColor * ase_lightColor * ase_lightColor.a );
			float4 OUTPUT_Albedo407 = ( lerpResult332 + temp_output_268_0 + temp_output_265_0 );
			float4 temp_output_408_0 = OUTPUT_Albedo407;
			o.Albedo = temp_output_408_0.rgb;
			float3 ase_vertex3Pos = mul( unity_WorldToObject, float4( i.worldPos , 1 ) );
			float3 objToWorldDir343 = mul( unity_ObjectToWorld, float4( ase_vertex3Pos, 0 ) ).xyz;
			float3 normalizeResult345 = normalize( objToWorldDir343 );
			float dotResult346 = dot( ase_worldlightDir , normalizeResult345 );
			...
			float4 lerpResult339 = lerp( ( temp_output_268_0 + temp_output_265_0 ) , OUTPUT_Albedo407 , SSS_Effect394);
			float4 OUTPUT_Emissive398 = lerpResult339;
			float4 temp_output_399_0 = OUTPUT_Emissive398;
			o.Emission = temp_output_399_0.rgb;
		}
...
/*ASEBEGIN
Version=17800
1920;120;1906;760;845.2688;27.70801;1.3;True;True
...
Node;AmplifyShaderEditor.CommentaryNode;406;-3766.658,-1337.893;Inherit;False;3050.46;2470.514;Comment;71;392;331;328;372;329;403;335;267;314;109;264;261;108;260;259;106;286;258;104;107;105;262;326;327;102;257;285;290;256;103;97;396;373;370;289;255;132;371;134;368;254;253;99;98;252;249;94;95;251;248;96;247;200;93;246;245;78;241;77;243;17;305;304;306;268;397;265;292;330;489;491;Color and Highlight Mixing;1,1,1,1;0;0
Node;AmplifyShaderEditor.RangedFloatNode;364;-582.5151,885.101;Float;False;Property;_NoisePower;NoisePower;30;0;Create;True;0;0;False;0;0;80;0;2000;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.SamplerNode;335;-2762.142,-606.1206;Inherit;True;Property;_VRTC;VRTC;9;0;Create;True;0;0;False;0;-1;782ef3ffdf428ce48934f5774562ea50;782ef3ffdf428ce48934f5774562ea50;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;6;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.RegisterLocalVarNode;403;-2261.331,-580.8635;Float;True;originalAlphaCutout;-1;True;1;0;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;114;-512.2765,424.4365;Float;False;Property;_BumpPower;BumpPower;17;0;Create;True;0;0;False;0;0.5;0.64;0;5;0;1;FLOAT;0
Node;AmplifyShaderEditor.SamplerNode;7;-73.77025,365.0654;Inherit;True;Property;_NormalMap;NormalMap;6;0;Create;True;0;0;False;0;-1;cf24829a9bef4734582a302bd6f6d130;19591b8e62d2fbc46bcc771f083d5bcc;True;0;True;bump;Auto;True;Object;-1;Auto;Texture2D;6;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.RangedFloatNode;300;-333.4024,588.1662;Float;False;Property;_Spread;Spread;29;0;Create;True;0;0;False;0;0;0.67;0;3;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;303;-4666.173,-818.6374;Float;False;Property;_MainHighlightPosition;Main Highlight Position;22;0;Create;True;0;0;False;0;0;0.41;-1;3;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;244;-3992.72,-1179.694;Float;False;Property;_SecondaryHighlightPosition;Secondary Highlight Position;26;0;Create;True;0;0;False;0;0;0.68;-1;3;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;368;-2729.725,-381.3201;Float;False;Property;_VRTC_Bias;VRTC_Bias;11;0;Create;True;0;0;False;0;0;0.42;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;371;-2728.873,-294.687;Float;False;Property;_VRTC_Scale;VRTC_Scale;12;0;Create;True;0;0;False;0;1;0.52;0;4;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.FunctionNode;370;-2232.945,-420.237;Inherit;False;ConstantBiasScale;-1;;7;63208df05c83e8e49a48ffbdce2e43a0;0;3;3;COLOR;0,0,0,0;False;1;FLOAT;0;False;2;FLOAT;0;False;1;COLOR;0
...
Node;AmplifyShaderEditor.RangedFloatNode;107;-2597.844,129.8814;Float;False;Property;_MainHighlightExponent;Main Highlight Exponent;24;0;Create;True;0;0;False;0;0.2;220;0;1000;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;348;-254.5041,1661.022;Float;False;Property;_Light_Scale;Light_Scale;2;0;Create;True;0;0;False;0;0;4.73;0;10;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.ColorNode;329;-2045.256,-1287.893;Float;False;Property;_RootColorPowerA;RootColor-Power(A);15;0;Create;True;0;0;False;0;0.1102941,0.1005623,0.1005623,0.453;0,0,0,1;False;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.ColorNode;326;-2428.317,-963.6812;Float;False;Property;_VariantColor1_GlossA;VariantColor1_Gloss(A);13;0;Create;True;0;0;False;0;0.8897059,0.1308391,0.1308391,0.516;0.4926471,0.2354563,0.2354563,0.403;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.ColorNode;327;-2427.24,-787.0815;Float;False;Property;_Variant_Color2_GlossA;Variant_Color2_Gloss(A);14;0;Create;True;0;0;False;0;0,0.1356491,0.7867647,0.516;0.7647059,0.5135008,0.3205017,0.416;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.RangedFloatNode;347;-247.6851,1548.879;Float;False;Property;_Light_Bias;Light_Bias;1;0;Create;True;0;0;False;0;0;0.0205;0;0.1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;262;-2662.638,673.6901;Float;False;Property;_SecondaryHighlightExponent;Secondary Highlight Exponent;28;0;Create;True;0;0;False;0;0.2;14;0;1000;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;351;118.6937,1671.82;Float;False;Property;_LightScatter;LightScatter;3;0;Create;True;0;0;False;0;0;0.372;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;259;-2644.141,764.4827;Float;False;Property;_SecondaryHighlightStrength;Secondary Highlight Strength;27;0;Create;True;0;0;False;0;0.25;0.469;0;2;0;1;FLOAT;0
Node;AmplifyShaderEditor.ColorNode;292;-1071.695,-941.9322;Float;False;Property;_TipColorPowerA;TipColor-Power(A);16;0;Create;True;0;0;False;0;0.9448277,1,0,0.484;0.1102941,0.07951019,0.0405493,0.331;False;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.RangedFloatNode;108;-2592.595,210.4834;Float;False;Property;_MainHighlightStrength;Main Highlight Strength;23;0;Create;True;0;0;False;0;0.25;1.264;0;2;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.ColorNode;264;-2017.542,831.6193;Float;False;Property;_SecondaryHighlightColor;Secondary Highlight Color;25;0;Create;True;0;0;False;0;0.8862069,0.8862069,0.8862069,0;0.9927992,1,0.4779412,1;False;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.ColorNode;267;-2323.583,-261.4672;Float;False;Property;_MainHighlight_Color;MainHighlight_Color;19;0;Create;True;0;0;False;0;0,0,0,0;0.6470588,0.5614187,0.5614187,1;False;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.RangedFloatNode;340;924.7745,1137.039;Float;False;Property;_BaseIllumination;BaseIllumination;4;0;Create;True;0;0;False;0;0.15;0.072;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.CommentaryNode;488;1832.548,-1354.126;Inherit;False;1902.575;752.6144;FresnelFX;13;473;475;474;469;472;481;479;476;471;470;484;482;483;;1,1,1,1;0;0
Node;AmplifyShaderEditor.RangedFloatNode;357;929.132,1229.432;Float;False;Property;_SSS;SSS;5;0;Create;True;0;0;False;0;0;0.273;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.CommentaryNode;458;2410.926,-2648.926;Inherit;False;1250.211;620.2566;LIGHT;10;457;456;454;455;486;453;485;487;452;451;;1,1,1,1;0;0
Node;AmplifyShaderEditor.RangedFloatNode;475;1885.548,-912.8106;Inherit;False;Property;_FresnelPower;FresnelPower;39;0;Create;True;0;0;False;0;0;0;0;2;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;474;1882.548,-996.8105;Inherit;False;Property;_FresnelScale;FresnelScale;37;0;Create;True;0;0;False;0;0;0;0;2;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;473;1891.548,-1073.811;Inherit;False;Property;_FresnelBias;FresnelBias;38;0;Create;True;0;0;False;0;0;0;-2;2;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;384;715.2823,694.3094;Float;False;Property;_XTiling;XTiling;33;0;Create;True;0;0;False;0;0;5.42;1;8;0;1;FLOAT;0
Node;AmplifyShaderEditor.LightAttenuation;453;2457.202,-2445.341;Inherit;False;0;1;FLOAT;0
Node;AmplifyShaderEditor.ColorNode;472;2636.482,-910.4252;Inherit;False;Property;_FresnelColourEffectA;FresnelColour-Effect(A);36;0;Create;True;0;0;False;0;0.6226415,0.2261481,0.2261481,0;0,0,0,0;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.RangedFloatNode;485;2500.164,-2544.074;Inherit;False;Constant;_Float0;Float 0;39;0;Create;True;0;0;False;0;1;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;487;2433.829,-2352.132;Float;False;Property;_CustomLightingShadow;CustomLightingShadow;21;0;Create;True;0;0;False;0;0.8;0.142;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.FresnelNode;469;2257.49,-1086.91;Inherit;False;Standard;WorldNormal;ViewDir;False;False;5;0;FLOAT3;0,0,1;False;4;FLOAT3;0,0,0;False;1;FLOAT;0;False;2;FLOAT;1;False;3;FLOAT;5;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;451;2444.204,-2140.926;Float;False;Property;_CustomLightingPower;CustomLightingPower;20;0;Create;True;0;0;False;0;0.1;0.142;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;396;-2230.989,-318.3039;Float;False;Property;_Alpha_Level;Alpha_Level;8;0;Create;True;0;0;False;0;0.3;1.4;0;2;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.ClampOpNode;479;2639.525,-1045.512;Inherit;False;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.LerpOp;486;2748.127,-2498;Inherit;False;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;454;2788.626,-2165.58;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;10;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;380;1379.835,734.2379;Float;False;Property;_Stretch;Stretch;32;0;Create;True;0;0;False;0;0.4961396;0.692;0.01;1.01;0;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;395;1058.093,106.4162;Inherit;False;394;SSS_Effect;1;0;OBJECT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;481;2920.525,-734.5119;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;3;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;397;-1904.882,-486.9533;Inherit;True;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.LightColorNode;452;2495.828,-2263.021;Inherit;False;0;3;COLOR;0;FLOAT3;1;FLOAT;2
Node;AmplifyShaderEditor.RangedFloatNode;491;-1954.161,-271.1568;Float;False;Property;_Alpha_Power;Alpha_Power;7;0;Create;True;0;0;False;0;1.8;1.4;0;2;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.PowerNode;490;-1693.161,-362.1568;Inherit;False;False;2;0;FLOAT;0;False;1;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;455;2944.686,-2410.173;Inherit;False;3;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.CommentaryNode;414;2500.923,-1963.795;Inherit;False;1730.291;638.9198;Comment;11;0;389;424;450;459;448;411;408;413;399;29;FINAL OUTPUTs;1,1,1,1;0;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;476;3003.691,-859.8885;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RegisterLocalVarNode;410;274.5262,345.2551;Float;False;OUTPUT_NormalMap;-1;True;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.PowerNode;391;1863.139,734.4855;Inherit;False;False;2;0;FLOAT;0;False;1;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;446;425.184,-1707.961;Float;False;Property;_PushHairs_Bias;PushHairs_Bias;35;0;Create;True;0;0;False;0;0;1;1;10;0;1;FLOAT;0
Node;AmplifyShaderEditor.ClampOpNode;456;3132.559,-2433.969;Inherit;False;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;471;3005.684,-1084.625;Inherit;False;2;2;0;FLOAT;0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
...
Node;AmplifyShaderEditor.ClampOpNode;489;-1528.161,-405.1568;Inherit;False;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;1;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;429;411.1858,-2084.723;Float;False;Property;_PushHairs;PushHairs;34;0;Create;True;0;0;False;0;0;0;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleAddOpNode;470;3207.979,-1108.373;Inherit;False;2;2;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
...
Node;AmplifyShaderEditor.RegisterLocalVarNode;457;3340.063,-2446.832;Float;False;LightAttInt;-1;True;1;0;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;29;2552.708,-1653.873;Float;False;Property;_Metallic;Metallic;18;0;Create;True;0;0;False;0;0;0.576;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.SamplerNode;374;2182.722,446.7526;Inherit;True;Property;_ExtraCuttingR;ExtraCutting(R);31;0;Create;True;0;0;False;0;-1;None;d33913761d8e47041b874944b7db3ec2;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;6;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.GetLocalVarNode;459;2951.554,-1806.91;Inherit;False;457;LightAttInt;1;0;OBJECT;;False;1;FLOAT;0
Node;AmplifyShaderEditor.CustomStandardSurface;448;2911.133,-1710.365;Inherit;False;Metallic;Tangent;6;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,1;False;2;FLOAT3;0,0,0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;1;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;450;3248.281,-1705.33;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT3;0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.SimpleAddOpNode;434;1595.011,-1846.243;Inherit;False;2;2;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.RangedFloatNode;484;3345.201,-1122.851;Inherit;False;Property;_Fresnel_PostEffect;Fresnel_PostEffect;40;0;Create;True;0;0;False;0;0.2;0;0;0.2;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleAddOpNode;482;3385.808,-1284.708;Inherit;False;2;2;0;FLOAT3;0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
...
Node;AmplifyShaderEditor.GetLocalVarNode;424;2567.712,-1407.594;Inherit;False;423;OUTPUT_VertexMotion;1;0;OBJECT;0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.LerpOp;483;3551.123,-1304.126;Inherit;False;3;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;2;FLOAT;0;False;1;COLOR;0
Node;AmplifyShaderEditor.StandardSurfaceOutputNode;0;3803.98,-1787.486;Float;False;True;-1;6;;0;0;CustomLighting;RRF_HumanShaders/HairShader2/HairShader2_AS_RRF_VRTC_CLight;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;False;False;False;False;False;False;Off;1;False;-1;3;False;-1;True;1;False;-1;5;False;-1;False;5;Custom;0.63;True;True;0;True;TransparentCutout;;Geometry;All;14;all;True;True;True;True;0;False;-1;False;0;False;-1;255;False;-1;255;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;0;False;-1;False;2;15;10;25;False;0.5;True;0;5;False;-1;10;False;-1;0;5;False;-1;2;False;-1;0;False;-1;0;False;-1;0;False;0;0,0,0,0;VertexOffset;True;False;Cylindrical;False;Absolute;0;;0;-1;-1;-1;0;True;0;0;False;-1;-1;0;False;-1;0;0;0;False;0.1;False;-1;0;False;-1;15;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;2;FLOAT3;0,0,0;False;3;FLOAT3;0,0,0;False;4;FLOAT;0;False;6;FLOAT3;0,0,0;False;7;FLOAT3;0,0,0;False;8;FLOAT;0;False;9;FLOAT;0;False;10;FLOAT;0;False;13;FLOAT3;0,0,0;False;11;FLOAT3;0,0,0;False;12;FLOAT3;0,0,0;False;14;FLOAT4;0,0,0,0;False;15;FLOAT3;0,0,0;False;0
...
WireConnection;469;2;474;0
WireConnection;469;3;475;0
...
WireConnection;490;0;397;0
WireConnection;490;1;491;0
WireConnection;455;0;486;0
WireConnection;455;1;452;2
...
WireConnection;470;0;408;0
WireConnection;470;1;471;0
WireConnection;457;0;456;0
WireConnection;392;0;489;0
WireConnection;374;1;381;0
WireConnection;448;0;470;0
...
WireConnection;482;0;450;0
WireConnection;482;1;470;0
...
WireConnection;0;13;483;0
WireConnection;0;11;424;0
ASEEND*/
//CHKSM=E6A2DC67D8AE7EE6B066468CDAB4161396787971